...
const FormDesign = () => {
  ...
  const [currentsDesignSelected, setCurrentsDesignSelected] =
    useState<currentDesignSelectedInterface>(defaultDesignSelected);

  useEffect(() => {
    const tabOption =
      tabRef.current && tabRef.current.querySelectorAll("input");
  ...
  useEffect(() => {
    ...
    allImages.forEach((element) => {
      element.onclick = () => {
        ...
        const currentDesignSelected: currentDesignSelectedInterface =
          currentsDesignSelected;
        ...
        if (element.classList.contains(style.active)) {
          ...
          if (typeof id !== "undefined" && typeof category !== "undefined") {
            currentDesignSelected[category] = 'default';
    ...
    <Accordion title="Editar design">
      <form className={style.formDesign}>
        ...
        <fieldset className={style.groupImages}>
          {selectImageGroup[images] &&
            selectImageGroup[images].map((item) => (
              <img
                ...
                className={
                  currentsDesignSelected[item.currentCategory] === item.name
                    ? style.active
                    : style.img
                }
              />
